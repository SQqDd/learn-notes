# Reflect 统一的对象操作 API
## 描述
- Reflect 是静态方法，不能 new
- 内部封装了一系列对对象的底层操作
- Reflect 成员方法就是 Proxy 处理对象的默认实现。（Proxy 处理对象的默认函数是 Reflect 的成员方法，他俩的处理方法名字是一样的）
- 统一提供一套用于操作对象的 API
- （有时候操作对象需要用到 Object.xxx，有时候需要用到 delete、in 等方法，Reflect 提供了一套统一的处理方法）
